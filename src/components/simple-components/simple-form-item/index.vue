<template>
  ...
  >
    <el-form-item
      :label="label"
      :label-width="labelWidth ? `${labelWidth}px` : undefined"
      :prop="prop"
      :rules="rule"
      ...
      <template v-if="!isSlot">
        ...
        <template v-if="inputType === 'date'">
          <el-date-picker
            ...
            range-separator="-"
            :start-placeholder="
              ['datetimerange', 'daterange'].includes($attrs.type) && '开始时间'
            "
            :end-placeholder="
              ['datetimerange', 'daterange'].includes($attrs.type) && '结束时间'
            "
          />
        </template>
...
const selectOption = computed(() => {
  ...
  // 字符串查找字典后返回
  if (isString(props.options)) return []
