<template>
  <div>
    ...
    <el-form-item label="图片字段:">
      <el-select v-model="componentOption.imgField" placeholder="请选择图片字段" style="width: 100%">
        <el-option
          v-for="(item, index) in dataSourceFields"
          :key="index"
    ...
    <el-form-item label="主要字段:">
      <el-select v-model="componentOption.field1" placeholder="请选择主要字段" style="width: 100%">
        <el-option
          v-for="(item, index) in dataSourceFields"
          :key="index"
          :label="`${item.label}（${item.key}）`"
    ...
    <el-form-item label="次要字段:">
      <el-select v-model="componentOption.field2" value-key="key" placeholder="请选择次要字段" style="width: 100%">
        <el-option
          v-for="(item, index) in dataSourceFields"
          :key="index"
          :label="`${item.label}（${item.key}）`"
    ...
    <el-form-item label="信息字段:">
      <el-select v-model="componentOption.useColumns" multiple value-key="key" placeholder="请选择信息字段" style="width: 100%">
        <el-option v-for="(item, index) in dataSourceFields" :key="index" :label="`${item.label}（${item.key}）`" :value="item" />
      </el-select>
    </el-form-item>
    <el-form-item label="列数:">
      <el-slider v-model="componentOption.span" :min="0" :max="4" />
    </el-form-item>
    ...
    <Modal v-model="showModal" title="自定义返回结果" @on-ok="handleOk">
      <div class="use-fields-list">
        <div class="use-fields-item" v-for="(item, index) in componentOption.useColumns" :key="index">
...
export default {
  ...
  methods: {
    ...
    clear() {
      this.componentOption.field1 = '';
      this.componentOption.field2 = '';
      this.componentOption.useColumns = [];
      this.componentOption.imgField = '';
    },
    handleOk() {
